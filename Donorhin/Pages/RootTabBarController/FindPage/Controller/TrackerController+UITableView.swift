...
extension TrackerController : UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        switch data.status {
        case .onGoing?: cell?.setupView(status: .onGoing, number: indexPath.row+1)
            break
        case .toDo?: cell?.setupView(status: .toDo, number: indexPath.row+1)
            break
        case .done?: cell?.setupView(status: .done, number: indexPath.row+1)
            break
        case .none:
            break
        }
